...
PlotAll = 1; % Flag to plot all time domain waveforms (slows down run immensely)
...
%     [ ys, t ] = SS_WF_Reconstruct( Xss(:,2), As(:,:,2), Bs(:,:,2), ts(2), u );
    Dx = abs(sign(As(:,:,2)*Xss(:,2) + Bs(:,:,2)*u) - sign(As(:,:,2)*Xss(:,3) + Bs(:,:,2)*u));
    overRes = Dx(1) == 2;%(min(ys(1,:)) < -Vdc);

    exitFlag = 0;

        while((diodcon1 || overRes)  || (hardswitching && exitFlag <20));
...
hold on;
plotSmithChart_lessLabels;
[C,h] = contour(X,Y,Z_LTSpice,30,'LineWidth',4);
h.LevelList = 10:10:100;
% plot(Greal, Gimag, 'ok')
% scatter(Greal(Pout<110), Gimag(Pout<110), [], Pout(Pout<110),'filled')
caxis([0, 100])
...
%%
figure(6);
Err = abs(Z-Z_LTSpice)./Z_LTSpice*100; 
Err(Z<10) = 0;
Err(Z>100) = 0;
h = polar([-pi pi], [0 1]);
delete(h);
hold on;
plotSmithChart_lessLabels;
% [C,h] = contourf(X,Y,Err,5,'LineWidth',4);
% hold on
Xvec = X(Z<90 & Z>10); Yvec = Y(Z<90 & Z>10); Errvec = Err(Z<90& Z>10);
