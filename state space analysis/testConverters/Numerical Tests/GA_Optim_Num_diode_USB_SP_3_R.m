...
LB = [0.005 0.005 1e6 0.001 0.001];
UB = [0.4 0.4 2e6 0.0075 0.0075];
sf = [20 20 1e-6 100 100];  %scale factor to ~ equalize magnitudes of variables
...
options = optimoptions('ga','PlotFcn',@gaplotbestf,'TolCon',1e-9,'Display','iter',...
    'MaxGenerations',100,'MaxTime',60*60*1, 'MaxStallGenerations',500, 'PopulationSize', 30);
options.InitialPopulation = [
    0.0816    0.2365    1.1015e6    0.005598    0.001303
    0.0650    0.1842    1.0662e6    0.004517    0.001069
    0.0538    0.1900    1.0743e6    0.004606    0.001177
    0.0650    0.1842    1.0662e6    0.004517    0.001069
    0.0574    0.1894    1.0557e6    0.004658    0.001056
    0.0875    0.2343    1.1199e6    0.005803    0.001619
    0.0939    0.2349    1.0315e6    0.005989    0.001089
    0.0906    0.2366    1.0473e6    0.005987    0.001034
    0.0869    0.2337    1.1198e6    0.005784    0.001496
    0.0816    0.2365    1.1015e6    0.005598    0.001303
    ].*sf;
% Add initial guess into initial population matrix to speed up process
[x,fval,exitflag,output,population,scores] = ...
    ga(@AURA_Eff_Sweep_IC_reduce_USB_SP_3_R, Nvars, A, b, Aeq, beq, LB.*sf, UB.*sf, [], INTCON, options);
...
%}
L23=900*10^-9;
widths_selection = linspace(0.1,0.4,10);
sf = [20 20 1e-6 100 100];
for i = 1:length(widths_selection)
    for j = 1:length(widths_selection)
    
        X = [widths_selection(i),widths_selection(j), 1.1015e6    0.005598    0.001303].*sf;
        Area_per_cell(i,j) = ([1 2 0 0 0].*L23./sf)*X';
        Area_total(i,j) = ([6 10 0 0 0].*L23./sf)*X';
        Resultz(i,j) = AURA_Eff_Sweep_IC_reduce_USB_SP_3_R(X);
end
