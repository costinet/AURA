...
LB = [0.005 0.005 0.005 0.005 0.005 1e6 0.001 0.001];
UB = [0.4 0.4 0.4 0.4 0.4 2e6 0.0075 0.0075];
sf = [20 20 20 20 20 1e-6 100 100];  %scale factor to ~ equalize magnitudes of variables
...
options = optimoptions('ga','PlotFcn',@gaplotbestf,'TolCon',1e-9,'Display','iter',...
    'MaxGenerations',100,'MaxTime',60*60*6, 'MaxStallGenerations',500, 'PopulationSize', 50);
options.InitialPopulation = [
     0.0794    0.0778    0.2511    0.2737    0.0289   1.0799e6    0.003669    0.001097
    0.0774    0.0861    0.2439    0.2680    0.0271   1.0747e6    0.004549    0.001492
    0.0794    0.0778    0.2511    0.2737    0.0289   1.0799e6    0.003669    0.001097
    
    ].*sf;
% Add initial guess into initial population matrix to speed up process
[x,fval,exitflag,output,population,scores] = ...
    ga(@AURA_Eff_Sweep_IC_reduce_USB_SP_3, Nvars, A, b, Aeq, beq, LB.*sf, UB.*sf, [], INTCON, options);
