...
try
    ...
    while i <= size(ONorOFF,2)
        ...
        if ~noviolation
            ...
            [Xon,Yon] = find(negInfx>-Vf); % Diode should be off
        end
        ...
        if ~isempty(min(Xon)<min(Xoff)) || isempty(Xoff)
           ...
           % end
            ...
            elseif min_Xon==1
                ...
                new_state = ONorOFF(:,i); % Find state that needs to be copied
                new_state(Yon(min_index)) = -1; % make correction in state
                ONorOFF(:,i) =  new_state; % Place new state space in the ONorOFF matrix
                [A,B,C,D,eigA_n] = obj.add_state_matrix(new_state); % Calculate the needed
...
%             end
            ...
            elseif i==1 && min_Xoff==1
                % If the inital condition needed to have a diode on
                %{
                min_index=find(Xoff==min_Xoff);
                ...
                % For flyback converter to keep output current flowing
                % correctly
                min_index=find(Xoff==min_Xoff);
                
                new_state = ONorOFF(:,i); % Find state that needs to be copied
                new_state(Yoff(min_index)) = 1; % make correction in state
                ONorOFF = [new_state ,ONorOFF(:,2:end) ]; % Place new state space in the ONorOFF matrix
                [A,B,C,D,eigA_n] = obj.add_state_matrix(new_state); % Calculate the needed
