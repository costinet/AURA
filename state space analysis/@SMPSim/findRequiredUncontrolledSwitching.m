...
%FINDREQUIREDUNCONTROLLEDSWITCHING is a fuctnion of the SMPSsim class where
%error type and location is determined

% adjType (:,:,1) is for when there is an after error
% adjType (:,:,2) is for when there is a before error
% tLocs shows which time intervals contain an error
...
%}
    ...
    switchRef = obj.converter.topology.switchRef;
...
%              (errBefore & (ints ~= circshift(ints,1)) & ~(circshift(errAfter,1,2)<0)) ;
    ...
    prev_state = circshift(obj.converter.swvec,1);
    current_prev_comp = obj.converter.swvec == prev_state;
    next_state = circshift(obj.converter.swvec,-1);
   current_next_comp = obj.converter.swvec == next_state;


    % This finds where there is a error before and after that crosses over
    % the same switching action. It then determines if the 
    

    
    [i1,j1] =  find(errBefore) ;
     
    [i2,j2] = find(circshift(errAfter,[0 1]));
...
%{
    if any(LIA)
        for i = 1:length(j1)
            if LIA(i)==1
                after_index = switchRef(i2(j2==j1(i)),1);
                before_index = switchRef(i1(j1==j1(i)),1);
                one_to_compare_to = current_next_comp(j1(i),:);
                compare_to=ones(size(one_to_compare_to));
                compare_to(before_index) = 0;
                if ~all(compare_to==one_to_compare_to)
                    errLocs(i1(i),j1(i)) = true;
                end
            end
        end
    end
%}
    ...
    % If there is a case where in one deadtime there are multiple diodes
    % that need to have different periods of time. Need to identify the
    ...
    adjustableButSameInterval = errAfter  & ...
        ints == circshift(ints,-1) &  errAfter.*switchRef(:,1) | 
        (errBefore & ...
        ((ints == circshift(ints,1)) & (obj.converter.swind == circshift(obj.converter.swind,1))));
    %}
