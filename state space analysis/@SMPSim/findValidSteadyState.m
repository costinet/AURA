...
try
    ...
    while(niter<limit)
        ...
        if debug2
            ...
            %obj.plotSomeOutputs(niter+500,[2,4,6,8,2+39,4+39,6+39,8+39])
            %obj.plotSomeOutputs(niter+600,[5,39+5])
        end
...
%%{
        if niter < 2
            if finalRun == 0
                ...
                violateMarginStart(:,:) = 0; % To only add diodes at the end of intervals to start
                violateMarginEnd(switchRef(:,1)==17|switchRef(:,1)==18,:) = 0; % To not have the steep ringing cause added solve time
            end
        end
...
%{
            tLocs(15,end-2:end) = [0 0 0];
            tLocs(15,1:3) = [0 0 0];
            tLocs(6,1:end-1) = 0;
%}
%}
%%{
        if altered==1 && niter ==1
            insertAt(:) = 0 ;
        end
%}
        ...
        for i = flip(find(insertAt))
            ...
            %[~, dts] = conv.getDeltaT(); 
            % took out min(dts) from calculation of dt to help converge
            % faster BAXTER
            dt = min([min(conv.controlledts)/10 , conv.ts(i)/10]);
            for j = 1:2
                if(any(adjType(:,i,j)))
                    [alt, newSwInd] = ...
