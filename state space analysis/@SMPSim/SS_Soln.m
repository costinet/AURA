...
% corresponding to a valid steady-state solution.
    ...
    % if(sum(sum(sum(As == obj.oldAs))) == numel(As)+10)
    %     fresp = obj.oldFresp;
    % else
        for i=1:n
    % % %        if(condAs(i) < 1e9) % not ill-conditioned, use matrix inverse
    % % %            fresp(:,:,i) = As(:,:,i)^-1*(expAs(:,:,i)-eye(ns))*(Bs(:,:,i)*u);
    ...
    % % % 
    % % %            if sum(sum(isnan(fresp(:,:,i)))) || sum(sum(isinf(fresp(:,:,i)))) || max(max(convError))>1e-9
    % % %                x0 = ones(1,ns)';
    % % %                [~,~,test3] = lsim(ss(As(:,:,i),Bs(:,:,i),zeros(1,ns),0),repmat(u',100,1), linspace(0,ts(i),100),x0);
    % % %                fresp(:,:,i) = test3(end,:)' - expAs(:,:,i)*x0;
    % % % 
    % % %                % If iterative summation didn't converge, try symbolic Laplace (very slow; last resort)
    % % %                if sum(sum(isnan(fresp(:,:,i)))) || sum(sum(isinf(fresp(:,:,i))))
    % % %                    syms s t
    % % %                    fresp(:,:,i) = eval(subs(vpa(ilaplace(((s*eye(ns)-As(:,:,i))^-1)/s)), t, ts(i)))*(Bs(:,:,i)*u);
    % % %                end
    % % %            end  
    % % %        end
