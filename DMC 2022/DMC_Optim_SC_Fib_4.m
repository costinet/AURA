...
try
    ...
    %x = [ 12.0000    6.0000   14.0000   15.0000    2.0000   1.2315e6    0.009    0.009];
    x = [15 15 15 15 15    1.2315    0.9000    0.9000];
    %x = x.*sf;
    ...
    while(big_loop<max_iteration)
        for i = 1:FETs_number
            ...
            [stick(i,1),graph1]=DMC_SC_Fib_4_Sweep(x,Coss_adj,Ron_adj);
            
            Coss_adj = [0 0 0 0 0 ].*adjust_FET;
            Ron_adj = [-1e-3 -1e-3 -1e-3 -1e-3 -1e-3 ].*adjust_FET;
            
            [stick(i,2),graph2]=DMC_SC_Fib_4_Sweep(x,Coss_adj,Ron_adj);
            
            
            Coss_adj = [-0.4e-9 -0.4e-9 -0.4e-9 -0.4e-9 -0.4e-9 ].*adjust_FET;
            Ron_adj = [0 0 0 0 0 ].*adjust_FET;
            
            [stick(i,3),graph3]=DMC_SC_Fib_4_Sweep(x,Coss_adj,Ron_adj);
    ...
    if I == 3 && the_sign == -1
        ...
        errorCoss = FETs(:,2)<=FETs(x(i),2);
            ...
            [stick_em,graph1]=DMC_SC_Fib_4_Sweep(x_test,Coss_adj_test,Ron_adj_test);
