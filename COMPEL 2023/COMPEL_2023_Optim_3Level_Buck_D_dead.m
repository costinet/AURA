...
% indDB.add(indDBig(logical(indDBindex)));
    ...
    % Latest: 9/21/22:
    x = [3.0000    3.0000    0.6667];
    % newest best 19.0000    8.0000    0.2000
    % x =[1  1   1  ];
    ...
    while(big_loop<max_iteration)
        ...
        for i = 1:FETs_number
            ...
            [stick(i,1),graph1]=COMPEL_2023_3Level_Buck_D_Sweep_dead(x,Coss_adj,Ron_adj,L_adj,RL_adj);
            
            Coss_adj = [0 ].*adjust_FET;
            Ron_adj = [deltaRon].*adjust_FET;
            
            [stick(i,2),graph2]=COMPEL_2023_3Level_Buck_D_Sweep_dead(x,Coss_adj,Ron_adj,L_adj,RL_adj);
            
            
            Coss_adj = [deltaCoss ].*adjust_FET;
            Ron_adj = [0 ].*adjust_FET;
            
            [stick(i,3),graph3]=COMPEL_2023_3Level_Buck_D_Sweep_dead(x,Coss_adj,Ron_adj,L_adj,RL_adj);
            ...
            [stick_em,graph1]=COMPEL_2023_3Level_Buck_D_Sweep_dead(x_test,Coss_adj_test,Ron_adj_test,L_adj_test,RL_adj_test);
          ...
          fs_pos = 3;
            ...
            for n=1:length(fs_sweep)
                ...
                [stick_fs(n),graph1]=COMPEL_2023_3Level_Buck_D_Sweep_dead(x,Coss_adj_test,Ron_adj_test,L_adj_test,RL_adj_test);
...
hold on
plot(plot_ron(saved_x(:,1)),plot_Coss(saved_x(:,1)),'DisplayName','M','LineWidth',3,'LineStyle','--');
hold on
scatter(plot_ron(x(:,1:1)),plot_Coss(x(:,1:1)),50,'r','DisplayName','Ending Point');
...
% Create title
title('3 Phase 3 Level Buck','FontSize',24);
