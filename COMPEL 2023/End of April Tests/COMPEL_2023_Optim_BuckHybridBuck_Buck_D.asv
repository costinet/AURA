...
try
    ...
    big_loop = 0;
    max_iteration = 50;
    almost_steady_state = [1 1 0 0];
    saved_x = [];
    sf = [1 1 1 1e-6];
    number_of_components = [3 3 3];
    %Area_adjust = 1e5;
    Area_adjust = 1e4; % Used as a factor to add area and Ploss for the parato front fuction

    % Latest: 9/21/22:

x = [15.0000    9.0000   18.0000    0.425];

%x = [8	21	18	0.3	];

    %x = [9.0000    17.0000   8.0000    0.50];
%x = [15.0000    13.0000    16.0000    0.50];

    % x = [9   17    59.0000    0.5000];
   % x = [9.0000    17.0000    3.0000    0.5000];
  % 9.0000    6.0000   43.0000    0.9167
    % new best  x = [21.0000   13.0000    8.0000    0.3250];
    %x =[1 2  1   1  ];
    % x = [21.0000   13.0000    8.0000    0.3250];
    ...
    for i  = 9
        for j  = 1:20
            for k  = 1:20
                ...
                x_test(2) = j;
                x_test(3) = k;
                [total_loss(i,j,k),graph1]=COMPEL_2023_BuckHybridBuck_D_Sweep(x_test,Coss_adj_test,Ron_adj_test,L_adj_test,RL_adj_test);
...
x_test = x;
Number_to_test = 3;
    total_loss = zeros(20,1);
    for i  = 1:20

                x_test(Number_to_test) = i;
                
                [total_loss(i),graph1]=COMPEL_2023_BuckHybridBuck_D_Sweep(x_test,Coss_adj_test,Ron_adj_test,L_adj_test,RL_adj_test);
...
total_losses_size = total_loss+FETs(1:20,3).*Area_adjust.*number_of_components(Number_to_test);


[V,I] = sort(total_losses_size);

%% Big while loop for optimization
    while(big_loop<max_iteration)

        %% Transistor Optimziation
        %%{
        for i = 1:2
            ...
            [stick(i,1),graph1]=COMPEL_2023_BuckHybridBuck_D_Sweep(x,Coss_adj,Ron_adj,L_adj,RL_adj);

            Coss_adj = [0 0 ].*adjust_FET;
            Ron_adj = [deltaRon deltaRon ].*adjust_FET;

            [stick(i,2),graph2]=COMPEL_2023_BuckHybridBuck_D_Sweep(x,Coss_adj,Ron_adj,L_adj,RL_adj);


            Coss_adj = [deltaCoss deltaCoss ].*adjust_FET;
            Ron_adj = [0 0].*adjust_FET;

            [stick(i,3),graph3]=COMPEL_2023_BuckHybridBuck_D_Sweep(x,Coss_adj,Ron_adj,L_adj,RL_adj);
            ...
            Projected_PlossArea = Projected_Ploss + FETs(:,3)'.*Area_adjust.*number_of_components(i);
            ...
            [stick_em,graph1]=COMPEL_2023_BuckHybridBuck_D_Sweep(x_test,Coss_adj_test,Ron_adj_test,L_adj_test,RL_adj_test);
...
%{
        for loop = 1:80
                x_test(i) = loop;
                [inductor_est(loop),graph1]=COMPEL_2023_BuckHybridBuck_D_Sweep(x_test,Coss_adj_test,Ron_adj_test,L_adj_test,RL_adj_test);
        end
%}
        ...
        fs_pos = 4;
            ...
            fs_sweep = linspace(100e3+x(fs_pos)*1e6,-100e3+x(fs_pos)*1e6,9);
        ...
        for n=1:length(fs_sweep)
            ...
            [stick_fs(n),graph1]=COMPEL_2023_BuckHybridBuck_D_Sweep(x,Coss_adj_test,Ron_adj_test,L_adj_test,RL_adj_test);
...
hold on
plot3(plot_ron(saved_x(:,1)),plot_Coss(saved_x(:,1)),plot_Area(saved_x(:,1)),'DisplayName','M1,2,3','LineWidth',3,'LineStyle','--');
hold on
plot3(plot_ron(saved_x(:,2)),plot_Coss(saved_x(:,2)),plot_Area(saved_x(:,2)),'DisplayName','M4,5,6','LineWidth',3,'LineStyle',':');
hold on
scatter3(plot_ron(x(:,1:2)),plot_Coss(x(:,1:2)),plot_Area(x(:,1:2)),50,'r','DisplayName','Optimal Solution');
...
hold on
plot3(plot_L(saved_x(:,3)),plot_RL(saved_x(:,3)),plot_AreaL(saved_x(:,3)),'DisplayName','L1,2,3','LineWidth',3,'LineStyle','--');
hold on
scatter3(plot_L(x(:,3)),plot_RL(x(:,3)),plot_AreaL(x(:,3)),50,'r','DisplayName','Ending Point');
...
% Create title
title('BuckHybridBuck','FontSize',24);
...
scatter(saved_x(2:end,4),total_area(2:end),'DisplayName','Test Points');
hold on
plot(saved_x(2:end,4),total_area(2:end),'DisplayName','fs','LineWidth',3,'LineStyle','--');
hold on
scatter(saved_x(end,4),total_area(end),50,'r','DisplayName','Ending Point');
...
% Create title
title('uckHybridBuck','FontSize',24);
