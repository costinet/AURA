...
%% Big while loop for optimization
    while(big_loop<max_iteration)
        ...
        Projected_PlossArea = Projected_Ploss + INDs(:,3)'.*Area_adjust;
...
%}
        ...
        fs_sweep = linspace(2e6,0.5e6,10);
...
hold on
plot3(plot_ron(saved_x(:,1)),plot_Coss(saved_x(:,1)),plot_Area(saved_x(:,1)),'DisplayName','M1,4,5,6','LineWidth',3,'LineStyle','--');
hold on
plot3(plot_ron(saved_x(:,2)),plot_Coss(saved_x(:,2)),plot_Area(saved_x(:,2)),'DisplayName','M2,3','LineWidth',3,'LineStyle',':');
hold on
...
% Create title
title('Buck Hybrid Buck Converter Optimization','FontSize',24);
