function [finalRun,totalerror]=COMPEL_2023_SteadyState(sim,conv,top)
% COMPEL_2023_SteadyState is used to find the steady state of the 

niter = 0;
debug = 0;
debug2 = 0;
totalerror = 1; %Initialize error incase there is an issue
...
debug = 0;
debug2 = 0;
max_niter = 100; % The maximum number of iterations before the function returns without reaching zero error

% TrTracker is used to determine how many times in a row the solution to 
% the Jocobain goes beyond the limits set by fastest resonance frequency if
% the number reaches beyond trLimit then those restrictions are ignored for
% 1 iteration to attempt to speed up the convergence.
...
%toc
totalerror = sum(errBefore + errAfter, 'all');
Xss = sim.steadyState;
if niter>max_niter
    J = 56123156423123;
end
