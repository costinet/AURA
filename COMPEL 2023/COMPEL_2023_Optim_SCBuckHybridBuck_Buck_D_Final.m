function [stick] = COMPEL_2023_Optim_SCBuckHybridBuck_Buck_D_Final(X)
...
try
    ...
    for j = 1:FETs_number
        for i = 1:Number_of_FETs
            ...
            FETs_AREA(i,j)  = FETs_in_parallel(j)*transDB(i).Width.approx*transDB(i).Length.approx;
    ...
    for j = 1:INDs_number
        for i = 1:Number_of_INDs
            ...
            INDs_AREA(i,j) = INDs_in_parallel(j)*indDB(i).Width.approx*indDB(i).Length.approx;
            % Used if I am looking a changing area here as well
        end
    end
    ...
    big_loop = 0;
    max_iteration = 50;
    almost_steady_state = [0];
    saved_x = [];
    sf = [ones(1,5), 1, 1e-6] ;


    %                  |   |
    x = [16 13 1 1 1 7  0.5 X];
    ...
    PerturbRon = zeros(FETs_number,2);
    PerturbCoss = zeros(FETs_number,2);
    dETAdRON = zeros(FETs_number,1);
    dETAdCOSS = zeros(FETs_number,1);
    ...
    [Inital_Eff]=COMPEL_2023_SCBuckHybridBuck_D_Sweep_Final(x,Coss_adj,Ron_adj,L_adj,RL_adj);
    ...
    while(big_loop<max_iteration)
        ...
        [Control_Eff]=COMPEL_2023_SCBuckHybridBuck_D_Sweep_Final(x,Coss_adj,Ron_adj,L_adj,RL_adj);

        tempx = x;
        x(frequencyIndex) = x(frequencyIndex)-deltaF;
        [PerturbF]=COMPEL_2023_SCBuckHybridBuck_D_Sweep_Final(x,Coss_adj,Ron_adj,L_adj,RL_adj);
        ...
        for i = 1:FETs_number
            ...
            [PerturbRon(i,:)]=COMPEL_2023_SCBuckHybridBuck_D_Sweep_Final(x,Coss_adj,Ron_adj,L_adj,RL_adj);

            Coss_adj = [deltaCoss deltaCoss deltaCoss deltaCoss deltaCoss].*adjust_FET;
            Ron_adj = [0 0 0 0 0].*adjust_FET;

            [PerturbCoss(i,:)]=COMPEL_2023_SCBuckHybridBuck_D_Sweep_Final(x,Coss_adj,Ron_adj,L_adj,RL_adj);
            ...
            ptdistM(:,:,i) = data-data(ic,:);

            dETAdRON(i)  = ((1-PerturbRon(i,2)) - (1-Control_Eff(2))) / (deltaRon*scaleFETs(1));
            dETAdCOSS(i)  = ((1-PerturbCoss(i,2)) - (1-Control_Eff(2))) / (deltaCoss*scaleFETs(2));
        end
        ...
        [PerturbL]=COMPEL_2023_SCBuckHybridBuck_D_Sweep_Final(x,Coss_adj,Ron_adj,L_adj,RL_adj);


        L_adj = [0];
        RL_adj = [deltaRL];

        [PerturbRL]=COMPEL_2023_SCBuckHybridBuck_D_Sweep_Final(x,Coss_adj,Ron_adj,L_adj,RL_adj);
       ...
       % Control_AREA = Control_Eff(1)*Iout
        minvecL = [dETAdL dETAdRL dETAdF];
        minvecM = [dETAdRON dETAdCOSS];

        for i = 1:FETs_number
            mindistM(:,i) = sum(ptdistM(:,:,i).^2,2) ./ sum(ptdistM(:,:,i).*minvecM(i,1:2),2);
            mindistM(mindistM < 0) = inf;
            [absdeltaDistM(:,i),nextptM(:,i)] = mink(abs(mindistM(:,i)),2);
            deltaDistM(:,i) = mindistM(nextptM(:,i),i);

           
            %Attempt at area stuff
           % Pojected_Eff_RON = (1-Control_Eff(2))+dETAdRON(i).*ptdistM(:,1,i);
           % Pojected_Eff_COSS = (1-Control_Eff(2))+dETAdCOSS(i).*ptdistM(:,2,i);
           % sqrt(Pojected_Eff_RON.^2+Pojected_Eff_COSS.^2)
           % OM_Projected_EFF_RON = 1-Pojected_Eff;
            %  = FETs_AREA(:,i)-FETs_AREA(x(i),i);

        end

        mindistL = sum(ptdistL.^2,2) ./ sum(ptdistL.*minvecL(1:2),2);
        mindistL(mindistL < 0) = inf;

        [absdeltaDistL,nextptL] = mink(abs(mindistL),2);
        deltaDistL = mindistL(nextptL);

        nextpt = [nextptM nextptL];
        deltaDist = [deltaDistM deltaDistL];


        for i = 1:FETs_number+INDs_number

            FETsDeltaArea = FETs_AREA(nextpt(:,1:5))-FETs_AREA(x(1:5));
            INDsDeltaAREA = INDs_AREA(nextpt(:,6))-INDs_AREA(x(6));

            if nextpt(1,i) == x(i)
                ACTnextpt(i) = nextpt(2,i);
                ACTdeltaDist(i) = deltaDist(2,i);
            else
                ACTnextpt(i) = nextpt(1,i);
                ACTdeltaDist(i) = deltaDist(1,i);
            end
            if isinf(ACTdeltaDist(i)) || isnan(ACTdeltaDist(i))
                dT = (1+.05*minvecL(3)*10);
                dT = min(max(dT,.5),1.5);
                ts = ts*dT;
                NoFETImprovFLAG = 1;
            end
            dT = (1+ACTdeltaDist(i)*minvecL(3)*10);
            dT = min(max(dT,.5),1.5);
            ts = ts*dT;
        end

        x_test = x;
        x_test(1:6) = ACTnextpt;
        ...
        [Tested_Eff]=COMPEL_2023_SCBuckHybridBuck_D_Sweep_Final(x_test,Coss_adj_test,Ron_adj_test,L_adj_test,RL_adj_test);

        if (Tested_Eff(2)>Control_Eff(2))||NoFETImprovFLAG
            % If there is not an improvement to the fval of the converter.
            almost_steady_state = 1;
        else
            x = x_test;
            saved_x(end+1,:) = x;
            almost_steady_state = 0;
...
%}
    ...
    [stick]=COMPEL_2023_SCBuckHybridBuck_D_Sweep_Final(x,Coss_adj,Ron_adj,L_adj,RL_adj);
