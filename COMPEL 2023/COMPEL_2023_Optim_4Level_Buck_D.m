...
indDB.add(indDBig(logical(indDBindex)));
    ...
    while(big_loop<max_iteration)
        for i = 1:FETs_number
            ...
            [stick(i,1),graph1]=COMPEL_2023_4Level_Buck_D_Sweep(x,Coss_adj,Ron_adj,L_adj,RL_adj);
            
            Coss_adj = [0 ].*adjust_FET;
            Ron_adj = [deltaRon].*adjust_FET;
            
            [stick(i,2),graph2]=COMPEL_2023_4Level_Buck_D_Sweep(x,Coss_adj,Ron_adj,L_adj,RL_adj);
            
            
            Coss_adj = [deltaCoss ].*adjust_FET;
            Ron_adj = [0 ].*adjust_FET;
            
            [stick(i,3),graph3]=COMPEL_2023_4Level_Buck_D_Sweep(x,Coss_adj,Ron_adj,L_adj,RL_adj);
    ...
    if I == 3 && the_sign == -1
        ...
        errorCoss = FETs(:,2)<=FETs(x(i),2);
            ...
            [stick_em,graph1]=COMPEL_2023_4Level_Buck_D_Sweep(x_test,Coss_adj_test,Ron_adj_test,L_adj_test,RL_adj_test);
