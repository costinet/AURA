...
options = optimoptions('ga','PlotFcn',@gaplotbestf,'TolCon',1e-9,'Display','iter',...
    'MaxGenerations',100,'MaxTime',60*60*2, 'MaxStallGenerations',100, 'PopulationSize', 30);
options.InitialPopulation = [
    
       8 10 10 20 0.7e6
       26.0000   10.0000   14.0000    21.334    0.6932e6

    ].*sf;
% Add initial guess into initial population matrix to speed up process
[x,fval,exitflag,output,population,scores] = ...
    ga(@COMPEL_2023_DualPhaseMultiIndHybrid_D_Sweep_GA, Nvars, A, b, Aeq, beq, LB.*sf, UB.*sf, [], INTCON, options);
