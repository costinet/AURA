...
% indDB.add(indDBig(logical(indDBindex)));
   ...
   % x = [9.0000    17.0000    18.0000    0.5000];
    ...
    %x =[1 2  1   1  ];
    x = [16.0000   13.0000    8.0000    0.500];
    ...
    while(big_loop<max_iteration)
        for i = 1:FETs_number
            ...
            [stick(i,1),graph1]=COMPEL_2023_BuckHybridBuck_D_Sweep_dead(x,Coss_adj,Ron_adj,L_adj,RL_adj);
            
            Coss_adj = [0 0 ].*adjust_FET;
            Ron_adj = [deltaRon deltaRon ].*adjust_FET;
            
            [stick(i,2),graph2]=COMPEL_2023_BuckHybridBuck_D_Sweep_dead(x,Coss_adj,Ron_adj,L_adj,RL_adj);
            
            
            Coss_adj = [deltaCoss deltaCoss ].*adjust_FET;
            Ron_adj = [0 0].*adjust_FET;
            
            [stick(i,3),graph3]=COMPEL_2023_BuckHybridBuck_D_Sweep_dead(x,Coss_adj,Ron_adj,L_adj,RL_adj);
            ...
            [stick_em,graph1]=COMPEL_2023_BuckHybridBuck_D_Sweep_dead(x_test,Coss_adj_test,Ron_adj_test,L_adj_test,RL_adj_test);
            ...
            for n=1:length(fs_sweep)
                ...
                [stick_fs(n),graph1]=COMPEL_2023_BuckHybridBuck_D_Sweep_dead(x,Coss_adj_test,Ron_adj_test,L_adj_test,RL_adj_test);
...
hold on
plot(plot_ron(saved_x(:,1)),plot_Coss(saved_x(:,1)),'DisplayName','M1,4,5,6','LineWidth',3,'LineStyle','--');
hold on
...
hold on
plot(plot_L(saved_x(:,3)),plot_RL(saved_x(:,3)),'DisplayName','L1,2,3','LineWidth',3,'LineStyle','--');
hold on
